...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [_centerImageView
            addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tap)]];
            
        [_adScrollView addSubview:_centerImageView];
        
        _rightImageView =
            [[UIImageView alloc] initWithFrame:CGRectMake(kAdViewWidth * 2, 0, kAdViewWidth, kAdViewHeight)];
...
- (void)setUpTime {
    if (_isNeedCycleRoll) {
        moveTimer = [NSTimer scheduledTimerWithTimeInterval:_adMoveTime
                                                     target:self
...
+ (id)adScrollViewWithFrame:(CGRect)frame
               imageLinkURL:(NSArray *)imageLinkURL
       pageControlShowStyle:(UIPageControlShowStyle)PageControlShowStyle {
...
+ (id)adScrollViewWithFrame:(CGRect)frame
               imageLinkURL:(NSArray *)imageLinkURL
        placeHoderImageName:(NSString *)imageName
       pageControlShowStyle:(UIPageControlShowStyle)PageControlShowStyle {
    ...
    AdView *adView =
        [AdView adScrollViewWithFrame:frame imageLinkURL:imageLinkURL pageControlShowStyle:PageControlShowStyle];
    adView.placeHoldImage = [UIImage imageNamed:imageName];
    return adView;
}

+ (id)adScrollViewWithFrame:(CGRect)frame
          localImageLinkURL:(NSArray *)imageLinkURL
       pageControlShowStyle:(UIPageControlShowStyle)PageControlShowStyle {
    NSMutableArray *imagePaths = [[NSMutableArray alloc] init];
    for (NSString *imageName in imageLinkURL) {
    ...
    AdView *adView =
        [AdView adScrollViewWithFrame:frame imageLinkURL:imagePaths pageControlShowStyle:PageControlShowStyle];
    return adView;
}
...
- (void)setimageLinkURL:(NSArray *)imageLinkURL {
    ...
    centerImageIndex = 0;
    rightImageIndex = 1;
    if (imageLinkURL.count == 1) { //只有一张图片
        rightImageIndex = 0;
...
- (void)setPageControlShowStyle:(UIPageControlShowStyle)PageControlShowStyle {
    ...
    if (PageControlShowStyle == UIPageControlShowStyleLeft) {
        ...
        _pageControl.frame = CGRectMake(kAdViewWidth - 20 * _pageControl.numberOfPages, kAdViewHeight - 40,
                                        20 * _pageControl.numberOfPages, 20);
    }
